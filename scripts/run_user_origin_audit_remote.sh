...
log() {
  printf '[user-origin-audit][%s] %s\n' "$(timestamp_utc)" "$*"
}

die() {
  printf '[user-origin-audit][%s][error] %s\n' "$(timestamp_utc)" "$*" >&2
  exit 1
}
...
container_output_dir="${USER_ORIGIN_AUDIT_CONTAINER_OUTPUT_DIR:-/tmp/user-origin-audit}"
host_output_dir_input="${USER_ORIGIN_AUDIT_OUTPUT_DIR:-backups/reports/user-origin-audit}"
host_output_dir="$(resolve_path "${host_output_dir_input}")"

log "Preparing user-origin audit in ${DEPLOY_PATH}"
...
if [ ! -f "scripts/user_origin_audit.py" ]; then
  die "Audit script not found in repository checkout: scripts/user_origin_audit.py"
fi
...
audit_log="${host_output_dir}/user-origin-audit-remote.log"
rm -f "${audit_log}"

log "Running user-origin audit script in app container"
set +e
docker compose -f "${compose_file}" exec -T app python - \
  --output-dir "${container_output_dir}" \
  < scripts/user_origin_audit.py 2>&1 | tee "${audit_log}"
...
container_md_path="$(
  awk -F= '
    {
      marker = "user_origin_audit_markdown="
      idx = index($0, marker)
      if (idx <= 0) {
        next
      }
...
log "User-origin audit completed."
echo "user_origin_audit_host_json=${host_json_path}"
echo "user_origin_audit_host_markdown=${host_md_path}"
echo "user_origin_audit_status=success"
